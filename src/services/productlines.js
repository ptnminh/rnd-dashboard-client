...
export const productlineService = {
  ...
  createOptimizedMockup: async (payload) => {
    try {
      const { data: result } = await apiClient.post(
        `/new-product-line-briefs/create-batch/optimized-mockup`,
        payload
      );
      if (result?.success === false) {
        if (result?.code === 403) {
  ...
  updateReadyToLaunch: async ({ uid, data }) => {
    try {
      const response = await apiClient.put(
  ...
  updateScreenshot: async ({ uid, data }) => {
    try {
      const response = await apiClient.put(
        `/new-product-line-briefs/${uid}/photography`,
        data
      );
      const { data: result } = response;
      if (result?.success === false) {
  ...
  updateOptimizedMockup: async ({ uid, data }) => {
    try {
      const response = await apiClient.put(
        `/new-product-line-briefs/${uid}/optimized-mockup`,
        data
      );
      const { data: result } = response;
      if (result?.success === false) {
